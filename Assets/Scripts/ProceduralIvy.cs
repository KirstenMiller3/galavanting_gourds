...
public class ProceduralIvy : MonoBehaviour {
    ...
    public void createIvy(RaycastHit hit) {
        ...
        GameObject ivy = new GameObject("Ivy " + rootCount);
    ...
    [ContextMenu("Generate Node Branch")]
    public void CreateIvyInNodes()
    {
        //Vector3 tangent = findTangentFromArbitraryNormal(Vector3.up);
        GameObject ivy = new GameObject("Ivy " + rootCount);
        Vector3 dir = _nodes[_currentNode + 1].transform.position - _nodes[_currentNode].transform.position;
       //dir.Normalize();
        List<IvyNode> nodes = createBranch(maxPointsForBranch, _nodes[_currentNode].transform.position, Vector3.up, dir);
        GameObject branch = new GameObject("Branch " + _currentNode);
        Branch b = branch.AddComponent<Branch>();
        if (!wantBlossoms)
        {
            b.init(nodes, branchRadius, branchMaterial);
        ...
        {
            b.init(nodes, branchRadius, branchMaterial, leafMaterial, leafPrefab, flowerMaterial, flowerPrefab, _currentNode == 0);
        }
        branch.transform.SetParent(ivy.transform);
    ...
    public GameObject AddIvyBranch(Vector3 origin, Vector3 target)
    {
        //Vector3 tangent = findTangentFromArbitraryNormal(Vector3.up);
        GameObject ivy = new GameObject("Root " + rootCount);
        Vector3 dir = target - origin;
        //dir.Normalize();
        List<IvyNode> nodes = createBranch(maxPointsForBranch, origin, Vector3.up, dir);
        GameObject branch = new GameObject("Branch " + _currentNode);
        Branch b = branch.AddComponent<Branch>();
        if (!wantBlossoms)
        {
